...
MODEL_SPECS = [
     ModelSpec(name="gpt_l4_h4_d384 MonarchDion", n_layer=4, n_head=4, n_embd=384),
]
...
class Muon(torch.optim.Optimizer):
    def __init__(
        ...
        row_alpha: float = ROW_ALPHA,
    ):
        if not 0.0 < row_alpha <= 1.0:
            raise ValueError("row_alpha must be in (0, 1]")
        defaults = {"lr": lr, "momentum": momentum, "row_alpha": row_alpha}
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for param in group["params"]:
                ...
                if buffer.ndim == 3:
                    # Например: (2, n, m) -> (2n, m)
                    nblocks, n, m = buffer.shape
                    flat = buffer.reshape(nblocks * n, m)

                    num_rows = max(1, math.ceil(row_alpha * flat.shape[0]))
                    row_idx = torch.randperm(flat.shape[0], device=flat.device)[:num_rows]

                    selected = flat[row_idx]              # (alpha * nblocks*n, m)
                    ortho_selected = Newton_Schulz(selected)

                    update_flat = torch.zeros_like(flat)
                    update_flat[row_idx] = ortho_selected
                    ortho = update_flat.reshape_as(buffer)
                    ...
                    scale = math.sqrt(reshaped.shape[0] / reshaped.shape[1])
